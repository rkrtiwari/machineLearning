...
svmModel <- svm(Species ~ ., data = myIris, kernal = "linear", scale = TRUE)
w <- t(svmModel$coefs) %*% svmModel$SV
b <- -svmModel$rho

sym <- ifelse(myIris$Species == "versicolor", "+", "x")
col <- ifelse(myIris$Species == "versicolor", "red", "green")

plot(scale(myIris$Petal.Length), scale(myIris$Petal.Width), pch = sym, col = col)
abline(a = -b/w[1,2], b = -w[1,1]/w[1,2], col = "blue", lty = 1)
legend(x=-2.45, y = 1.85, legend = c("versicolor", "virginica", "decision boundary"), pch = c("+", "x", "_"), col = c("red", "green", "blue"), bty = "n")
```
