...
## Data Preparation
set.seed(100)
theta1 <- runif(40, min = 0, max = 2*pi)
r1 <- rnorm(40, mean=0, sd=1)
c1x <- 1
c1y <- 1
X1 <- c1x + r1*cos(theta1)
Y1 <- c1y + r1*sin(theta1)
#plot(X1,Y1)

set.seed(200)
theta2 <- runif(40, min = 0, max = 2*pi)
r2 <- rnorm(40, mean=0, sd=1)
c2x <- 3
c2y <- 1
X2 <- c2x + r2*cos(theta2)
Y2 <- c2y + r2*sin(theta2)
#plot(X2,Y2)

df1 <- data.frame(X = X1, Y = Y1, group = "group1")
df2 <- data.frame(X = X2, Y = Y2, group = "group2")
df <- rbind(df1,df2)
```


```{r, echo=FALSE, results='hide'}
#plot(df$X, df$Y, pch = 19, col = df$group, xlab = "X", ylab = "Y", 
#     xlim = c(-1,5), ylim = c(-1,5))
#legend(4,4, c("group1", "group2"), col = c("black", "red"), pch = c(19,19))
```
...
include_graphics("randomForest.jpg")
```
